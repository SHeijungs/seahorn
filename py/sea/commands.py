...
class ClangWithLibUAF(sea.LimitedCmd):
    """
    Like the Clang command, but link with the library for UAF instrumentation
    """

    def __init__ (self, quiet=False, plusplus=False):
        super (ClangWithLibUAF, self).__init__('clangWithLibUAF', 'Compile with libUAF', allow_extra=True)
        self.clangCmd = None
        self.plusplus = plusplus

    def mk_arg_parser (self, ap):
        ap = super (ClangWithLibUAF, self).mk_arg_parser (ap)
        ap.add_argument ('-m', type=int, dest='machine',
                         help='Machine architecture MACHINE:[32,64]', default=32)
        ap.add_argument ('-g', default=False, action='store_true',
                         dest='debug_info', help='Compile with debug info')
    ...
    def run (self, args, extra):
        ...
        if args.machine == 32:
          libUAF = os.path.join (lib_dir, 'libUAF-32.bc')
        else:
          libUAF = os.path.join (lib_dir, 'libUAF-64.bc')
        args.in_files.append(libUAF)
        out_files = []
        # create private workdir
        workdir = createWorkDir ()
        for f in args.in_files:
...
class instrUAF(sea.LimitedCmd):
    def __init__(self, quiet=False):
        super(instrUAF, self).__init__('instrUAF', 'Add UAF check instrumentation',
                                       allow_extra=True)
